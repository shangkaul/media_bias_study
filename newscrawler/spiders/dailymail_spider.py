...
class DailyMailSpider(SitemapSpider):
    ...
    custom_settings = {
        ...
        'LOG_LEVEL': 'DEBUG',
        'LOG_FILE': os.path.join(LOG_DIR, f'dailymail_spider_{date}.log'),
        'LOG_FORMAT': '%(asctime)s [%(name)s] %(levelname)s: %(message)s',
        'FEEDS': {
            os.path.join(DATA_DIR, f'dailymail_articles_{date}.json'): {
                'format': 'json',
                'encoding': 'utf8',
                'store_empty': False,
                'indent': 2
    ...
    SITE_CONFIG = {
        # article fields
        'title_path': '//div[contains(@id, "js-article-text")]/h1[1]/text()',
        'description_path': '//ul[contains(@class, "mol-bullets-with-font")]/li/text()',
        'date_path': '//span[contains(@class, "article-timestamp-published")]//time//text()',
        'author_path': '//p[contains(@class, "author-section")]//text()',
        'text_path': '//div[contains(@itemprop, "articleBody")]//text()',
    }
    ...
    def create_start_urls(self):
        ...
        base = 'https://www.dailymail.co.uk/sitemap-articles-day'
