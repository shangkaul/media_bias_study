...
class USATodaySpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'LOG_LEVEL': 'INFO',
        'LOG_FILE': os.path.join(LOG_DIR, f'usatoday_spider_{date}.log'),
        'LOG_FORMAT': '%(asctime)s [%(name)s] %(levelname)s: %(message)s',
        'FEEDS': {
            os.path.join(DATA_DIR, f'usatoday_articles_{date}.json'): {
                'format': 'json',
                'encoding': 'utf8',
                'store_empty': False,
                'indent': 2
    ...
    SITE_CONFIG = {
        # article fields
        'title_path': '//div[contains(@class, "story-topper")]//h1//text()',
        'text_path': '//article[contains(@class, "primary-content")]//p//text()',
        'date_path': '//div[contains(@class, "publish-date")]//lit-timestamp/@publishdate',
        'author_path': '//div[contains(@class, "attribution")]//span[contains(@class, "author")]//a//text()',

        'image_set_path': '//media-image/@image-set',
        'caption_path': '//media-image/@caption',

        # article links 
        'article_link_path': '//li[contains(@class, "sitemap-list-item")]//a/@href'
    }
    ...
    def create_start_urls(self):
        ...
        base = 'https://www.usatoday.com/sitemap'
        ...
        dates = {
            ...
            2025 : {
                'january' : [1, 31], 
                'february' : [1, 28], # 2025 is not a leap year
                'march' : [1, 31], 
                'april' : [1, 30], 
                'may' : [1, 31]
            }
    ...
    def __init__(self, *args, **kwargs):
        super(USATodaySpider, self).__init__(*args, **kwargs)
    ...
    def parse_article(self, response):
        ...
        image_sets = response.xpath(self.SITE_CONFIG['image_set_path']).getall()
        captions = response.xpath(self.SITE_CONFIG['caption_path']).getall()

        images = []
        captions_cleaned = []

        for imgset, cap in zip(image_sets, captions):
            best_crop_url = imgset.split(',')[0].replace(' bestCrop', '').strip()
            if best_crop_url and best_crop_url not in images:
                images.append(best_crop_url)
                captions_cleaned.append(cap.strip() if cap else '')
