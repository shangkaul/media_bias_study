...
def sanitize_filename(url: str) -> str:
    return unquote(Path(urlparse(url).path).name) or str(uuid.uuid4())
    ...
    for attempt in range(2):
        try:
            ...
            if resp.status_code == 429 and attempt == 0:
                logger.warning("429 on %s, backing off 10s…", url)
                time.sleep(10)
                continue
...
# ─── PARAMETERS ──────────────────────────────────────────────────────────────
INPUT_JSON           = "../bbc_independent_filtered.json"
OUTPUT_JSON          = "downloaded_images_index_bbc_ind.json"
IMG_ROOT             = Path("img_bbc_ind")
DRY_LIMIT_PER_SOURCE = None    # or int to cap per‐source downloads
MAX_WORKERS          = 10      # bump this up if you want more parallelism
...
workers = min(MAX_WORKERS, len(by_source))
